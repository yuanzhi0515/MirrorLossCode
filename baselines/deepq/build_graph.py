...
def build_train(make_obs_ph, q_func, num_actions, optimizer, grad_norm_clipping=None, gamma=1.0,
    ...
    mirror_coef=0.09, mirror=False, env_name=None):
...
`       See the top of the file for details.
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        if mirror:
            ...
            q_t_mirror_modified = mirror_modify(q_t_mirror, game=env_name)
            mirror_loss_unpri = tf.reduce_mean(tf.square(q_t_mirror_modified - q_t), 1)
        ...
        if mirror:
            mirror_loss = 0.09 * tf.reduce_mean(importance_weights_ph * mirror_loss_unpri)
